    ...
    def observe(self, observation, gameState):
        ...
        * When a ghost is captured by Pacman, all beliefs should be
        ...
        for p in self.legalPositions:
            if noisyDistance is None and p == self.getJailPosition():
                allPossible[p] = 1.0
            elif noisyDistance is None or p == self.getJailPosition():
                allPossible[p] = 0.0
            else:
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old_p in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, old_p))
            for p in self.legalPositions:
                new_beliefs[p] += newPosDist[p] * self.beliefs[old_p]
