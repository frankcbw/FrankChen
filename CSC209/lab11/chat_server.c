...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    usernames[user_index].username[num_read] = '\0';
...
int read_from(int client_index, struct sockname *usernames) {
    ...
    int num_read = read(fd, message, BUF_SIZE);
    message[num_read] = '\0';
    if (num_read == 0) {
        usernames[client_index].sock_fd = -1;
        return fd;
    }
    strncpy(buf, usernames[client_index].username, BUF_SIZE);
    strncat(buf, ": ", BUF_SIZE-strlen(usernames[client_index].username));
    strncat(buf, message, BUF_SIZE-strlen(buf));
...
int main(void) {
    ...
    for (int index = 0; index < MAX_CONNECTIONS; index++) {
        ...
        usernames[index].username = malloc(sizeof(char)*(BUF_SIZE+1));
    }
