...
char **prune_word_list(char **words, int len, int *words_remaining) {
    ...
    *words_remaining = 0;
    ...
    char **pruned = malloc(sizeof(char*) * (*words_remaining + 1));
    i = 0;
    while(words[i] != NULL) {
      if (strlen(words[i]) == len) {
        pruned[j] = malloc(sizeof(char) * (len+1));
        strncpy(pruned[j], words[i], len);
        j++;
      }
      i++;
    }
    pruned[*words_remaining] = NULL;
    return pruned;
}
...
char **get_word_list_of_length(char **words, int *len) {
    ...
    int words_remaining;
    char **word_list = NULL;
    char *str_len = malloc(sizeof(char) * MAX_LEN);
    while (word_list == NULL) {
      printf("Length of words to use? ");
      fgets(str_len, MAX_LEN, stdin);
      *len = (int)strtol(str_len, NULL, 10);
      if (*len <= 0) {
        printf("There are no words of that length.\n");
      }
      else {
